...
The first thing is to: imagine the shape of your classes. This `Codec` should be defined in [proto3](https://developers.google.com/protocol-buffers/docs/proto3) syntax. There are a number of different `int encodings`, `byte slices`, `strings`, and nested structures. And fields may be repeated. So forget complex types with methods now and just focus on the actual data structure.
...
The [x/codec.proto](https://github.com/iov-one/tutorial/blob/master/x/orderbook/codec.proto#L75-L88) file defines the `Trade` type rather simply, once you remove the comments, this is all that is left:
